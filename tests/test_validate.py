...
def test_simple_image_build(tmpdir, mocker):
    mocker.patch.object(sys, 'argv', ['concreate', '-v', 'build'])

    image_dir = str(tmpdir.mkdir('source'))

    with open(os.path.join(image_dir, 'image.yaml'), 'w') as outfile:
        yaml.dump(image_descriptor, outfile, default_flow_style=False)

    run_concreate(image_dir)


def test_simple_image_test(tmpdir, mocker):
    mocker.patch.object(sys, 'argv', ['concreate', '-v', 'build', 'test'])
