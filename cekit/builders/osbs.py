...
class OSBSBuilder(Builder):
    ...
    def prepare(self, descriptor):
        ...
        self.artifacts = [a.target_file_name()
                          for a in descriptor.get('artifacts', []) if not isinstance(a, _PlainResource)]
        # When plain artifact was handled using lookaside cache, we need to add it too
        # TODO Rewrite this!
        self.artifacts += [a.target_file_name()
                           for a in descriptor.get('artifacts', []) if isinstance(a, _PlainResource) and a.get('lookaside')]
